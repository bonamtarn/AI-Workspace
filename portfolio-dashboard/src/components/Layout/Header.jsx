...
export default function Header({ onAddAsset, onImportWallet }) {
  ...
  const btnCls = 'flex items-center gap-1.5 px-3 py-2 text-xs rounded-lg transition-all text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700'

  return (
    <div className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-6 py-3.5 flex items-center justify-between shrink-0">
      <div>
        <div className="flex items-center gap-3">
          <h1 className="text-lg font-bold text-slate-900 dark:text-white">{activePortfolio?.name}</h1>
          <span className="text-xs text-slate-400 dark:text-slate-500 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full border border-slate-200 dark:border-transparent">
      ...
      <div className="flex items-center gap-2">
        ...
        <button onClick={refreshPrices} disabled={isRefreshing} className={`${btnCls} disabled:opacity-40`}>
        ...
        <button onClick={onAddAsset}
          className="flex items-center gap-1.5 px-4 py-2 text-xs text-white bg-blue-600 hover:bg-blue-500 rounded-lg transition-all font-medium border border-blue-700">
