  ...
  const ValueCell = ({ thbValue }) => {
    ...
    return (
      <div className="flex flex-col">
        <span className="text-xs sm:text-sm font-medium text-slate-900 dark:text-white">{formatUSD(thbValue / usdToThb)}</span>
        <span className="text-xs text-slate-400 hidden sm:block">{formatTHB(thbValue)}</span>
      </div>
    )
  }
        ...
        <td className="px-2 sm:px-4 py-2.5">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="min-w-0">
              <div className="font-semibold text-slate-900 dark:text-white text-sm">{a.symbol}</div>
              <div className="text-xs text-slate-400 truncate max-w-[80px] hidden sm:block">{a.name}</div>
            </div>
            <span className={`text-xs px-1.5 py-0.5 rounded-full shrink-0 hidden sm:inline-flex ${cfg?.bgColor}`}>
              {cfg?.label}
            </span>
          </div>
        </td>
        <td className="px-2 sm:px-4 py-2.5 hidden md:table-cell"><PriceCell price={a.price} /></td>
      ...
      <tr key={`agg-${key}`}
        ...
        <td className="px-2 sm:px-4 py-2.5">
          <div className="flex items-center gap-2">
            ...
            <div className="min-w-0">
              <div className="font-semibold text-slate-900 dark:text-white text-sm">{first.symbol}</div>
              <div className="text-xs text-slate-400 hidden sm:block">{first.name} · {group.length} positions</div>
              <div className="text-xs text-slate-400 sm:hidden">{group.length} pos</div>
            </div>
            <span className={`text-xs px-1.5 py-0.5 rounded-full shrink-0 hidden sm:inline-flex ${cfg?.bgColor}`}>
              {cfg?.label}
            </span>
        ...
        <td className="px-2 sm:px-4 py-2.5 hidden md:table-cell"><PriceCell price={first.price} /></td>
        <td className="px-2 sm:px-4 py-2.5 hidden md:table-cell">
        ...
        <td className="px-2 sm:px-4 py-2.5 hidden sm:table-cell" onClick={e => e.stopPropagation()} />
      </tr>
    )
          ...
          <thead>
            <tr className="border-b border-slate-100 dark:border-slate-800 text-xs text-slate-400 dark:text-slate-500 uppercase tracking-wider">
              {[
                ...
                <th key={i} className={`px-2 sm:px-4 py-2.5 text-left font-semibold ${h.cls}`}>{h.label}</th>
              ))}
            </tr>
          </thead>
